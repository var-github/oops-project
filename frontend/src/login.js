...
const MIN_PHONE_LENGTH_E164 = 13; // +91 (3 chars) + 10 digits = 13 chars (clean E.164 format)
...
// --- CUSTOM INPUT COMPONENT (UPDATED FOR DUAL SPACING) ---
const CustomPhoneNumberInput = React.forwardRef(({ value, onChange, ...rest }, ref) => {

  // 1. Helper to clean input (remove spaces)
  const clean = (val) => (val ? val.replace(/\s/g, '') : '');

  // 2. Format for Display: "+91 99999 99999"
  const formatDisplayValue = (val) => {
    const cleanVal = clean(val);

    // Strict Length Check (Ensures no more than 10 digits are processed)
    ...
    // Apply formatting strictly for Indian numbers (+91)
    if (cleanedValue.startsWith('+91') && cleanedValue.length > 3) {
      const countryCode = cleanedValue.slice(0, 3); // +91
      const localNumber = cleanedValue.slice(3); // 10 digits

      if (localNumber.length > 5) {
        // Format: +91 [space] 99999 [space] 99999
        return `${countryCode} ${localNumber.slice(0, 5)} ${localNumber.slice(5)}`;
      }

      // If less than 5 digits, just show space after code: +91 9999
  ...
  // 3. Handle Typing/Changes
  const handleChange = (e) => {
    const inputValue = e.target.value;
    const cleanInput = clean(inputValue);

    // Block input if it exceeds the max length defined above
    if (cleanInput.startsWith('+91') && cleanInput.length > MIN_PHONE_LENGTH_E164) {
        return;
    }
...
function Login() {
  ...
  // Helper to check if the phone number is valid for submission (13 chars E.164)
      ...
      return (
        <form onSubmit={handleProfileSetup}>
          ...
          <p style={{textAlign: 'center', fontSize: '0.9rem', color: '#555', marginBottom: '15px'}}>
             {auth.currentUser?.email ? `Linked: ${auth.currentUser.email}` : 'Enter details'}
          </p>
    ...
    if (showOtpInput) {
      return (
        <form onSubmit={handleVerifyOtp}>
          ...
          <button type="button" onClick={()=>{setShowOtpInput(false); setError('');}} className="btn-primary btn-secondary">
            Back
          </button>
        </form>
      );
        ...
        <form onSubmit={handleSendOtp}>
          ...
          <div className="phone-input-container">
             <PhoneInput
                ...
                inputComponent={CustomPhoneNumberInput} // <--- CUSTOM FORMATTING
              />
          </div>
          <button
            type="submit"
            disabled={loading || !isValidPhoneNumber} // <--- BUTTON DISABLED UNTIL VALID LENGTH IS MET
    ...
    <div className="full-page-container">
      <style>
        ...
        * { box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        /* The Input Field inside PhoneInput */
